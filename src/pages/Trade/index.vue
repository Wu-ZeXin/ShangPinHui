  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" 
           v-for="(user,index) in addressList" :key="user.id"
      >
...
<script>
  export default {
    ...
    methods:{
      async getDate() {
        let result = await this.$API.reqTradeCartList()
        if(result.code == 200) {
          this.orderInfo = result.data
        }

        let resultAddress = await this.$API.reqUserAddressList()
        if(resultAddress.code == 200) {
          this.addressList = resultAddress.data
        }
      },
    ...
    computed:{
      defaultAddress() {
        ...
        // return this.addressList.map((item) => { if(item.isDefault == 1) return item })
