<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item"
               v-for="(record, index) in orderInfo.records" :key="record.id"
        >
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(orderDetail, index) in record.orderDetailList" :key="orderDetail.skuId">
              ...
              <td :rowspan="record.orderDetailList.length" v-if="index == 0" width="8%" class="center"></td>
    ...
    <Pagination :pageNo='page' :pageSize='limit' :total='orderInfo.total' :continues='5' @currentPage='changePage'/>
    </div>
    <!--猜你喜欢-->
    <div class="like">
      <h4 class="kt">猜你喜欢</h4>
...
export default {
  ...
  methods: {
    async getData() {
      let result = await this.$API.reqOrderInfo(this.page, this.limit);
      if (result.code == 200) {
